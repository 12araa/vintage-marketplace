...
const handleBuyNow = async () => {
  if (!isLoggedIn.value) {
    ...
    Swal.fire({
      icon: 'warning',
      title: 'Oops...',
      text: 'Please login first to buy this item!',
  ...
  if (product.value) {
    try {
      await store.dispatch('cart/addToCart', product.value);
...
<template>
  <div class="product-detail-page">
    <StoreAlert :visible="showSuccessModal" :product="product" @close="handleContinue" @toCart="handleToCart" />
      ...
      <div class="row mb-5">
        ...
        <div class="col-lg-7 mb-4">
          <div class="card border-0 rounded-3 overflow-hidden shadow-sm">
            <img :src="product.image" class="img-fluid w-100" style="object-fit: cover; max-height: 600px;"
              :alt="product.name">
          </div>
        </div>
        ...
        <div class="col-lg-5 ps-lg-4">
          ...
          <div class="d-grid gap-2 mb-4">
            <button @click="handleBuyNow" class="btn btn-success text-white py-2 fw-bold"
              style="background-color: #008080; border: none;">Buy
              Now</button>
            <button @click="handleAddToCart" class="btn btn-outline-success py-2 fw-bold"
              style="color: #008080; border-color: #008080;">Add to Cart</button>
          </div>

          <div class="card border-0 shadow-sm p-3 d-flex flex-row align-items-center bg-light">
            <img src="https://ui-avatars.com/api/?name=Jack+Corner&background=random" class="rounded-circle me-3"
              width="50" height="50">
            <div>
              ...
              <form @submit.prevent="submitReview">
                <div class="mb-2">
                  <div class="d-flex gap-1">
                    <i v-for="star in 5" :key="star" @click="setRating(star)" class="bi fs-5 cursor-pointer"
                      :class="star <= reviewForm.rating ? 'bi-star-fill text-warning' : 'bi-star text-secondary'"></i>
                  </div>
                </div>
                <div class="mb-2">
                  <textarea v-model="reviewForm.comment" class="form-control form-control-sm" rows="2"
              ...
              <div v-for="review in reviews" :key="review.id" class="border-bottom pb-3 my-3">
                ...
                <div class="mb-1">
                  <i v-for="n in 5" :key="n" class="bi"
                    :class="n <= review.rating ? 'bi-star-fill text-warning' : 'bi-star text-muted'"
